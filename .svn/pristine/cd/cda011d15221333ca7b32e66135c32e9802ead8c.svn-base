...
public class ServiceInspectionImpl implements ServiceInpectionService {
			...
			if (serviceInspection != null
				...
				if (serviceInspection.getChallanStatus().equals(InventoryConstant.YES)) {
					...
					if (inventoryAssign != null) {
						Integer test1 = inventoryAssign.getAllocatedQty() - serviceInspection.getAcceptedQuantity();
					...
					Integer test = inventoryEntrance.getQuantity() - serviceInspection.getAcceptedQuantity();
	...
	public DispatcherLog mapDispatchLogEntity(ServiceInspectionDto serviceInspectionDto, Principal principal,
		 ...
		 * dispatcherLog.setInventoryEntrance(serviceInspection.
		 * getInventoryEntrance()); }
		 */

		if (serviceInspection.getInventoryAssign() != null) {
			dispatcherLog.setInventoryAssign(serviceInspection.getInventoryAssign());
			//InventoryAssign inventoryEntrance = serviceInspection.getInventoryAssign();
			//if (inventoryEntrance != null) {
				//Integer test = inventoryEntrance.getAllocatedQty() - serviceInspection.getAcceptedQuantity();
				//System.out.println(test);
				//inventoryEntrance.setAllocatedQty(test);
				//inventoryAssignRepository.saveAndFlush(inventoryEntrance);
			//}
		}
	...
	public ServiceTracker mapServiceTrackerEntity(ServiceInspectionDto serviceInspectionDto, Principal principal,
		...
		serviceTracker.setAvailableQty(String.valueOf(serviceInspection.getQuantity()));
	...
	public ResponseDto receiveServiceTools(ServiceTrackerDto serviceTrackerDto, Principal principal,
		...
		serviceTracker.setAvailableQty(serviceTrackerDto.getAvailableQty());

		int remainQty = 0;

		if (serviceTracker.getAvailableQty() != null) {

			remainQty = Integer.parseInt(serviceTracker.getAvailableQty())
					- Integer.parseInt(serviceTrackerDto.getAvailableQty());
		}
		...
		if (Integer.parseInt(serviceTracker.getAvailableQty()) >= Integer
				.parseInt(serviceTrackerDto.getAvailableQty())) {
		...
		if (hasUserRole) {
			...
			//InventoryAssign inventoryAssign = inventoryAssignRepository.findOne(inv);
			
			InventoryAssign inventoryAssign = inventoryAssignRepository
					.findOne(serviceTracker.getInventoryAssign().getId());

			int test = inventoryAssign.getAvailableQty() + Integer.parseInt(serviceTrackerDto.getAvailableQty());
			System.out.println(test);
			if(test>0){
				inventoryAssign.setAvailableQty(test);
				}
			int test1 = inventoryAssign.getAllocatedQty()+ Integer.parseInt(serviceTrackerDto.getAvailableQty());
			if(test1>0){
			inventoryAssign.setAllocatedQty(test1);
			}
			inventoryAssignRepository.saveAndFlush(inventoryAssign);
		...
		if (hasWarehouseRole) {
			...
			int test = inventoryEntrance.getAvailableQuantity() + Integer.valueOf(serviceTrackerDto.getAvailableQty());
					
					if(test>0){
						
						inventoryEntrance.setAvailableQuantity(test);
					}
					int test1 = inventoryEntrance.getQuantity() + Integer.valueOf(serviceTrackerDto.getAvailableQty());
					if(test1>0){
						
						inventoryEntrance.setQuantity(test1);
					}
			inventoryStockRepository.saveAndFlush(inventoryEntrance);
