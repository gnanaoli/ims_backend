ims.controller('employeeManagementController',function($scope,$http,InsertService,$filter,FetchService,$location,$timeout,DTOptionsBuilder,UrlConstants) {
	...
	//$scope.employeeDto.isAddressSame=true;
...
$scope.empreadonly = true;
$scope.empreadonly1 = true;
	...
	$scope.saveEmployee=function(EmployeeDto)
	{
		var count=0;
		...
		if(EmployeeDto)
			...
			if(angular.isDefined($scope.employeeDto.id))
				...
				//alert("inside save new employee");
	...
	$scope.editView = function(){
		$scope.empreadonly = false;
		$scope.empreadonly1 = false;
		$scope.employeeDto.isAddressSame = false;
		//$scope.read=true;
		//$scope.empreadonly = false;
	}
	...
	// if($scope.employeeDto.isAddressSame !=null)
	{
		console.log(same);
    ...
    .withButtons([            
	   ...
	   /* for validateing confirmation date*/
	    $scope.validateConfirmationDate = function()
		{
		  ...
		  var dateAr = $scope.employeeDto.dateOfJoin.split('/');
			var startDate = dateAr[2] + '-' + dateAr[1] + '-' + dateAr[0].slice(-2);
			
			var dateAr1 = $scope.employeeDto.confirmationDate.split('/');
			var endDate = dateAr1[2] + '-' + dateAr1[1] + '-' + dateAr1[0].slice(-2);
			
			if(startDate > endDate)
				{
				$scope.employeeDto.confirmationDate="";
			}
