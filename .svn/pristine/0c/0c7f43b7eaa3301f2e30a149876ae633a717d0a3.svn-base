...
public class ToolsRequestServiceImpl implements ToolsRequestService {
	 ...
	 **/
	@Override
	public ResponseDto saveToolsRequest(InventoryRequestDto inventoryRequestDto, Principal principal) {
		...
		try {
			...
			inventoryRequest.setInventoryNumber(inventoryRequestDto.getInventoryNumber());
			if (inventoryRequestDto.getId() != null) {
				...
				if (inventoryRequest != null) {
					...
					responseDto.setNumber(inventoryRequest.getReferenceNo());
	...
	@Override
	public ResponseDto saveAcceptToolsRequest(InventoryRequestDto inventoryRequestDto, Principal principal)
			throws Exception {
		// String status = InventoryConstant.FAILURE;
		...
		if (inventoryRequest.getStatus().equals(InventoryConstant.ACCEPTED)
				|| inventoryRequest.getStatus().equals(InventoryConstant.PARTIALLYACCEPTED)) {
		...
		if (inventoryRequestDto.getStatus().equalsIgnoreCase(InventoryConstant.ACCEPTED)
			...
			if (inventoryRequest.getLocation().equalsIgnoreCase(InventoryConstant.WAREHOUSE)) {
				...
				// System.out.println("entrance quantity" +
				// inventoryEntrance.getQuantity());
				if (inventoryEntrance.getAvailableQuantity() >= inventoryRequestDto.getQuantity()) {
					...
					inventoryRequest = mapInventoryRequest(inventoryEntrance, inventoryAssign, inventoryRequest,
							inventoryRequestDto, principal);
					inventoryRequest.setInventoryEntrance(inventoryEntrance);
					inventoryRequest = toolsRequestRepository.saveAndFlush(inventoryRequest);
					/*
					 * if (inventoryRequest != null) { status =
					 * mapInventoryAssignEntity(inventoryAssign,
					 * inventoryEntrance, inventoryRequestDto, principal); if
					 * (status.equals(InventoryConstant.SUCCESS)) { status =
					 * InventoryConstant.SUCCESS; }
					 * 
					 * }
					 */
					if (inventoryRequest != null) {
						responseDto = saveDispatchLogEntity(inventoryAssign, inventoryEntrance, inventoryRequestDto,
								principal);
						// status = saveDispatchLogEntity(inventoryAssign,
						// inventoryEntrance, inventoryRequestDto,
						// principal);
						// if (status != null) {
						// status = InventoryConstant.SUCCESS;
						// }
				...
				if (inventoryAssign.getAvailableQty() >= inventoryRequestDto.getQuantity()) {
					...
					throw new Exception(InventoryConstant.INSUFFICIENT_QUANTITY);
					/*
					 * status = InventoryConstant.INSUFFICIENT_QUANTITY; return
					 * status;
					 */
				}
				...
				if (hasUserRole) {
					...
					List<InventoryEntrance> inventoryEntrances = inventoryStockRepository
							.getMaxQuantity(inventoryRequestDto.getSubCategory(), inventoryRequestDto.getRequestedTo());

					/*
					 * inventoryEntrances.sort((InventoryEntrance
					 * st1,InventoryEntrance
					 * st2)->st1.getInventoryNumber().compareTo(st2.
					 * getInventoryNumber()));
					 * inventoryEntrances.forEach((s)->System.out.println(s.
					 * getInventoryNumber()));
					 */

					inventoryEntrances = inventoryEntrances.stream()
							.sorted(Comparator.comparing(InventoryEntrance::getInventoryNumber))
							.collect(Collectors.toList());

					// System.out.println(inventoryEntrances);
					int requiredQty = inventoryRequestDto.getQuantity();

					int remainingQty, updatedQty, total;

					remainingQty = requiredQty;

					total = 0;

					List<InventoryEntrance> inventoryEntrances2 = new ArrayList<>();

					for (int i = 0; i < inventoryEntrances.size(); i++) {
						inventoryEntrance = inventoryEntrances.get(i);
						if (inventoryEntrance != null) {

							if (inventoryEntrance.getAvailableQuantity() >= remainingQty) {

								updatedQty = inventoryEntrance.getAvailableQuantity() - remainingQty;
								total = total + remainingQty;
								if (updatedQty > 0) {
									inventoryEntrance.setAvailableQuantity(updatedQty);
									inventoryEntrance.setLastModifiedBy(principal.getName());
									inventoryEntrance.setLastModifiedDate(new Timestamp(new Date().getTime()));
									inventoryEntrances2.add(inventoryEntrance);
								}

								else {
									inventoryEntrance.setAvailableQuantity(0);
									inventoryEntrance.setLastModifiedBy(principal.getName());
									inventoryEntrance.setLastModifiedDate(new Timestamp(new Date().getTime()));
									inventoryEntrances2.add(inventoryEntrance);
								}
								...
								updatedQty = inventoryEntrance.getAvailableQuantity() - remainingQty;
								total = total + inventoryEntrance.getAvailableQuantity();
								if (updatedQty < 0) {
									remainingQty = remainingQty - inventoryEntrance.getAvailableQuantity();
									inventoryEntrance.setAvailableQuantity(0);
									inventoryEntrance.setLastModifiedBy(principal.getName());
									inventoryEntrance.setLastModifiedDate(new Timestamp(new Date().getTime()));
									inventoryEntrances2.add(inventoryEntrance);
					...
					if (total == requiredQty) {
						...
						try {
							for (int i = 0; i < inventoryEntrances2.size(); i++) {
								inventoryEntrance = inventoryEntrances2.get(i);

								inventoryRequest = mapInventoryRequest(inventoryEntrance, inventoryAssign,
										inventoryRequest, inventoryRequestDto, principal);
								inventoryRequest.setInventoryEntrance(inventoryEntrance);
								// inventoryRequest =
								// toolsRequestRepository.saveAndFlush(inventoryRequest);

								Integer noOfRequest = toolsRequestRepository.updateAcceptedQty(
										inventoryRequest.getAcceptedQuantity(), inventoryRequest.getStatus(),
										inventoryRequest.getId());

								Integer noOfCounts = toolsRequestRepository.getAllQuantity(
										inventoryEntrance.getAvailableQuantity(), inventoryEntrance.getId());

								if (noOfRequest != null) {
									responseDto = saveDispatchLogEntity(inventoryAssign, inventoryEntrance,
											inventoryRequestDto, principal);
								}
							}
						} catch (Exception e) {
				...
				{
					if (hasUserRole1) {
						List<InventoryAssign> inventoryAssigns = inventoryAssignRepository.getMaxQuantity(
								inventoryRequestDto.getSubCategory(), inventoryRequestDto.getRequestedTo());
						if (inventoryAssigns != null) {
							inventoryAssigns = inventoryAssigns.stream()
									.sorted(Comparator.comparing(InventoryAssign::getInventoryNumber))
									.collect(Collectors.toList());
							...
							List<InventoryAssign> inventoryAssigns2 = new ArrayList<>();

							for (int i = 0; i < inventoryAssigns.size(); i++) {
								inventoryAssign = inventoryAssigns.get(i);
								if (inventoryAssign != null) {

									if (inventoryAssign.getAvailableQty() >= remainingQty) {

										updatedQty = inventoryAssign.getAvailableQty() - remainingQty;
										total = total + remainingQty;
										if (updatedQty > 0) {
											inventoryAssign.setAvailableQty(updatedQty);
											inventoryAssign.setLastModifiedBy(principal.getName());
											inventoryAssign.setLastModifiedDate(new Timestamp(new Date().getTime()));
											inventoryAssigns2.add(inventoryAssign);
										}

										else {
											inventoryAssign.setAvailableQty(0);
											inventoryAssign.setLastModifiedBy(principal.getName());
											inventoryAssign.setLastModifiedDate(new Timestamp(new Date().getTime()));
											inventoryAssigns2.add(inventoryAssign);
										}
										...
										updatedQty = inventoryAssign.getAvailableQty() - remainingQty;
										total = total + inventoryAssign.getAvailableQty();
										if (updatedQty < 0) {
											remainingQty = remainingQty - inventoryAssign.getAvailableQty();
											inventoryAssign.setAvailableQty(0);
											inventoryAssign.setLastModifiedBy(principal.getName());
											inventoryAssign.setLastModifiedDate(new Timestamp(new Date().getTime()));
											inventoryAssigns2.add(inventoryAssign);
							...
							if (total == requiredQty) {
								...
								try {
									for (int i = 0; i < inventoryAssigns2.size(); i++) {
										inventoryAssign = inventoryAssigns2.get(i);
										...
										Integer noOfCounts = toolsRequestRepository.getAllQuantity1(
												inventoryAssign.getAvailableQty(), inventoryAssign.getId());
							...
							/*
							 * if (inventoryAssign != null &&
							 * inventoryAssign.getAvailableQty() != null &&
							 * inventoryRequestDto.getQuantity() != null &&
							 * inventoryAssign.getAvailableQty() >=
							 * inventoryRequestDto.getQuantity()) { remaining =
							 * inventoryAssign.getAvailableQty() -
							 * inventoryRequestDto.getQuantity();
							 * inventoryAssign.setAvailableQty(remaining);
							 * inventoryAssign.setLeftoverQty(remaining);
							 * inventoryAssign.setLastModifiedBy(principal.
							 * getName());
							 * inventoryAssign.setLastModifiedDate(new
							 * Timestamp(new Date().getTime()));
							 * inventoryRequest =
							 * mapInventoryRequest(inventoryEntrance,
							 * inventoryAssign, inventoryRequest,
							 * inventoryRequestDto, principal);
							 * inventoryRequest.setInventoryAssign(
							 * inventoryAssign); inventoryRequest =
							 * toolsRequestRepository.saveAndFlush(
							 * inventoryRequest); if (inventoryRequest != null)
							 * { responseDto =
							 * saveDispatchLogEntity(inventoryAssign,
							 * inventoryEntrance, inventoryRequestDto,
							 * principal); } } else { throw new
							 * Exception(InventoryConstant.INSUFFICIENT_QUANTITY
							 * ); }
							 */
						}
	...
	public List<InventoryRequestDto> getUserToolsRequest(Principal principal) {
		...
		if (principal.getName() != null) {
			...
			if (user.getProjectItem() != null && !user.getProjectItem().isEmpty()) {
				...
				if (projectEntityList != null) {
					...
					for (int k = 0; k < projectEntityList.size(); k++) {
						...
						if (project != null) {
							List<InventoryRequest> inventoryRequestList = toolsRequestRepository
									.findByStatusAndRequestedTo(InventoryConstant.REQUESTED, project.getName());
			...
			if (user.getWareHouseItem() != null && !user.getWareHouseItem().isEmpty()) {
				...
				if (wareHouseEntityList != null) {
					...
					for (int k = 0; k < wareHouseEntityList.size(); k++) {
						...
						if (wareHouse != null) {
							List<InventoryRequest> inventoryRequestList = toolsRequestRepository
									.findByStatusAndRequestedTo(InventoryConstant.REQUESTED,
			...
			List<InventoryRequest> inventoryRequestList = toolsRequestRepository
					.findByStatusAndLocation(InventoryConstant.BULK_REQUESTED, InventoryConstant.ALL);
			if (inventoryRequestList != null && !inventoryRequestList.isEmpty()) {
				// inventoryRequestDtoList = new ArrayList<>();
				for (int i = 0; i < inventoryRequestList.size(); i++) {
					InventoryRequest inventoryRequest = inventoryRequestList.get(i);
					if (inventoryRequest != null
							&& !inventoryRequest.getRaisedBy().equalsIgnoreCase(principal.getName())) {
						inventoryRequestDtoList.add(mapInventoryRequestDtoById(inventoryRequest));
					}
				}
			}
			 ...
			 * !user.getWareHouseItem().isEmpty()) { wareHouseEntityList =
			 * user.getWareHouseItem(); if (wareHouseEntityList != null) {
			 * wareHouseDtoList = new ArrayList<>(); List<InventoryRequest>
			 * inventoryRequestList =
			 * toolsRequestRepository.findByStatusAndLocation(InventoryConstant.
			 * BULK_REQUESTED,InventoryConstant.ALL); if (inventoryRequestList
			 * != null && !inventoryRequestList.isEmpty()) {
			 * 
			 * List<InventoryRequest> inventoryRequestList =
			 * toolsRequestRepository.findByStatusAndLocation(InventoryConstant.
			 * BULK_REQUESTED,InventoryConstant.ALL); if (inventoryRequestList
			 * != null && !inventoryRequestList.isEmpty()) {
			 * //inventoryRequestDtoList = new ArrayList<>(); for (int i = 0; i
			 * < inventoryRequestList.size(); i++) { InventoryRequest
			 * inventoryRequest = inventoryRequestList.get(i); if
			 * (wareHouseEntityList != null) { wareHouseDtoList = new
			 * ArrayList<>(); for (int k = 0; k < wareHouseEntityList.size();
			 * k++) { WareHouse wareHouse = user.getWareHouseItem().get(k); if
			 * (wareHouse != null) { if (inventoryRequest != null &&
			 * !wareHouse.getWareHouseName().equalsIgnoreCase(inventoryRequest.
			 * getRequestedFrom())) {
			 * inventoryRequestDtoList.add(mapInventoryRequestDtoById(
			 * inventoryRequest)); } } } } } }
			 * 
			 * }
			 * 
			 * if (user.getProjectItem() != null &&
			 * !user.getProjectItem().isEmpty()) { projectEntityList =
			 * user.getProjectItem(); if (projectEntityList != null ) {
			 * projectDtoList = new ArrayList<>(); for (int k = 0; k <
			 * projectEntityList.size(); k++) { Project project =
			 * user.getProjectItem().get(k); if (project != null) {
			 * List<InventoryRequest> inventoryRequestList =
			 * toolsRequestRepository.findByStatusAndLocation(InventoryConstant.
			 * BULK_REQUESTED,InventoryConstant.ALL); if (inventoryRequestList
			 * != null && !inventoryRequestList.isEmpty()) {
			 ...
			 * !project.getName().equalsIgnoreCase(inventoryRequest.
			 * getRequestedFrom())) {
			 * inventoryRequestDtoList.add(mapInventoryRequestDtoById(
			 * inventoryRequest)); } } } } } } }
			 */
	...
	public InventoryRequestDto mapInventoryRequestDtoById(InventoryRequest inventoryRequest) {
		...
		try {
			logger.info(
					"<--ToolsRequestServiceImpl-------------mapInventoryRequestDtoById--------------------started-->");
			...
			if (inventoryRequest.getExpectedDateToReach() != null) {
				inventoryRequestDto.setExpectedDateToReach(
						MasterDateUtil.convertDateToString(inventoryRequest.getExpectedDateToReach()));
			}
	...
	public String mapInventoryAssignEntity(InventoryAssign inventoryAssign, InventoryEntrance inventoryEntrance,
			InventoryRequestDto inventoryRequestDto, Principal principal) {
		...
		try {
			...
			if (inventoryAssign.getInventoryEntrance() != null) {
				InventoryEntrance inventoryEntrance2 = inventoryStockRepository
						.findOne(inventoryAssign.getInventoryEntrance().getId());
	...
	public ResponseDto saveDispatchLogEntity(InventoryAssign inventoryAssign, InventoryEntrance inventoryEntrance,
			InventoryRequestDto inventoryRequestDto, Principal principal) {
		...
		try {
			...
			if (inventoryEntrance.getCategory().getAssetTracking().equals(InventoryConstant.INDIVIDUAL_TRACKING)) {

				dispatchLog.setAcceptedQuantity(1);
				dispatchLog.setAvailableQuantity(1);
				dispatchLog.setQuantity(1);

			} else {
				dispatchLog.setAcceptedQuantity(inventoryRequestDto.getQuantity());
			...
			if (location != null) {
				...
				dispatchLog.setInventoryNumber(inventoryEntrance.getInventoryNumber());
			}
			...
			if (location != null) {
				...
				dispatchLog.setInventoryNumber(inventoryAssign.getInventoryNumber());
			}
			...
			// }
			// } else if (inventoryEntrance != null) {
			// InventoryEntrance inventoryEntrance2 =
			// inventoryStockRepository.findOne(inventoryEntrance.getId());
			// if (inventoryEntrance2 != null) {
			// dispatchLog.setInventoryEntrance(inventoryEntrance2);
			// }
			// }
			 ...
			 * dispatchLog.setWareHouseSiteName(inventoryRequestDto.
			 * getRequestedTo());
			 */
			...
			if (dispatchLog != null) {
				// status = InventoryConstant.SUCCESS;
				responseDto.setStatus(InventoryConstant.SUCCESS);
				responseDto.setNumber(inventoryRequestDto.getReferenceNo());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public InventoryRequest mapInventoryRequest(InventoryEntrance inventoryEntrance, InventoryAssign inventoryAssign,
			InventoryRequest inventoryRequest, InventoryRequestDto inventoryRequestDto, Principal principal) {
		...
		try {
			 ...
			 * inventoryRequest.setDispatchDate(MasterDateUtil.
			 * convertStringToTimeStampDate(inventoryRequestDto.getDispatchDate(
			 * )));
			 * 
			 * } if(inventoryEntrance!=null){
			 * inventoryRequest.setInventoryEntrance(inventoryEntrance); }
			...
			// inventoryRequest.setAcceptedQuantity(inventoryEntrance.getAvailableQuantity());

			inventoryRequest.setActionBy(principal.getName());
			inventoryRequest.setActionDate(new Timestamp(new Date().getTime()));

			/*
			 * inventoryRequest=toolsRequestRepository.saveAndFlush(
			 * inventoryRequest);
			 */
	...
	public String saveMoveTools(ServiceTrackerDto serviceTrackerDto, Principal principal) throws Exception {
		...
		logger.info(
				"<--Tools Request serviceImpl-------------save Move tool after service--------------------started-->");
		...
		if (serviceTrackerDto.getNeedChallan().equalsIgnoreCase(InventoryConstant.YES)) {
			...
			dispatcherLog.setQuantity(Integer.parseInt(serviceTrackerDto.getQuantity()));
			...
			/*
			 * if(serviceTrackerDto.get) dispatcherLog.setInventoryNumber();;
			 * dispatcherLog.set dispatcherLog.
			 */
		...
		logger.info(
				"<--Tools Request serviceImpl-------------save Move tool after service--------------------ended-->");
		return status;
