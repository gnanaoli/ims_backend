...
public class DispatchLogManagementServiceImpl implements DispatchLogService {
	...
	public List<DispatcherLogDto> getAllDispatchLog(Principal principal) {
		...
		try {
			...
			if (authentication.getAuthorities().stream()
				...
				if (warehouseDtoList != null) {
					...
					for (int i = 0; i < warehouseDtoList.size(); i++) {
						...
						if (wareHouseDto != null) {
							List<DispatcherLog> dispatcherLogEntityList = dispatchLogRepository
				...
				if (role) {
					List<ProjectDto> projectDtoList = inventoryManagementService
							.getCurrentUserProjectDtoList(principal);
					...
					if (projectDtoList != null) {
						...
						for (int i = 0; i < projectDtoList.size(); i++) {
							...
							if (projectDto != null) {
								List<DispatcherLog> dispatcherLogEntityList = dispatchLogRepository
										.findByRequestedFrom(projectDto.getName());
								if (dispatcherLogEntityList != null) {
									for (int j = 0; j < dispatcherLogEntityList.size(); j++) {
										DispatcherLog dispatcherLog = dispatcherLogEntityList.get(j);
										if (dispatcherLog != null) {
	...
	public ResponseDto addInventoryInDispatch(BulkRequestResponseDto bulkRequestResponseDto, Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------addInventoryInDispatch--------------------started-->");
		...
		try {
			...
			if (bulkRequestResponseDto != null) {
				...
				for (int i = 0; i < bulkRequestResponseDto.getInventoryEntranceItemDto().size(); i++) {
					InventoryEntranceDto inventoryEntranceDto = bulkRequestResponseDto.getInventoryEntranceItemDto()
							.get(i);
					if (inventoryEntranceDto != null) {
						if (inventoryEntranceDto.getAllocatableQuantity() > 0 && inventoryEntranceDto.getSelected()) {
							InventoryEntrance inventoryEntrance = inventoryStockRepository
									.findOne(bulkRequestResponseDto.getInventoryEntranceItemDto().get(i).getId());
							if (inventoryEntrance.getAvailableQuantity() != null
									&& inventoryEntrance.getAvailableQuantity() > 0) {
								inventoryEntrance.setAvailableQuantity(inventoryEntrance.getAvailableQuantity()
										- inventoryEntranceDto.getAllocatableQuantity());
								...
								entityManager.merge(mapDispatcherEntity(bulkRequestResponseDto, inventoryEntranceDto,
										principal, referenceNo));
							}
						}
					}
				}
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------addInventoryInDispatch--------------------Ended-->");
		return responseDto;
	}

	@Override
	public ResponseDto addInventoryInDispatchForWareHouse(BulkRequestResponseDto bulkRequestResponseDto,
			String moveTowarehouse, String referenceNo, Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------addInventoryInDispatchForWareHouse--------------------started-->");
		ResponseDto responseDto = new ResponseDto();
		try {
				...
				for (int i = 0; i < bulkRequestResponseDto.getInventoryEntranceItemDto().size(); i++) {
					...
					if (inventoryEntranceDto != null) {
						if (inventoryEntranceDto.getAllocatableQuantity() > 0) {
							entityManager.merge(mapDispatcherEntityForWareHouseMove(bulkRequestResponseDto,
									inventoryEntranceDto, principal, referenceNo));
						}
					}
				}
				entityManager.flush();
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------addInventoryInDispatchForWareHouse--------------------Ended-->");
		return responseDto;
	}
	...
	public List<DispatcherLogDto> getAllDispatcherLogByCount(Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------getAllDispatcherLogByCount--------------------started-->");
		List<DispatcherLogDto> dispatcherLogDtoList = new ArrayList<>();
		try {
			...
			if (warehouseDtoList != null && warehouseDtoList.size() > 0) {
				...
				for (int i = 0; i < warehouseDtoList.size(); i++) {
					WareHouseDto wareHouseDto = warehouseDtoList.get(i);
					if (wareHouseDto != null) {
						List<Object> objectList = dispatchLogRepository
								.getAvailableQuantityCount(wareHouseDto.getWareHouseName());
						if (objectList != null) {
							for (int j = 0; j < objectList.size(); j++) {
								...
								if (objectArray[1] != null) {
									...
									dispatcherLogDto.setRequestedTo("");
								}
						...
						if (objectList != null) {
							for (int j = 0; j < objectList.size(); j++) {
								...
								if (objectArray[1] != null) {
									...
									dispatcherLogDto.setRequestedTo(InventoryConstant.SHOP);
								}
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------getAllDispatcherLogByCount--------------------Ended-->");
		return dispatcherLogDtoList;
	}

	public DispatcherLogDto mapDispatcherLogDto(DispatcherLog dispatcherLog) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatcherLogDto--------------------started-->");
		DispatcherLogDto dispatcherLogDto = new DispatcherLogDto();
		...
		dispatcherLogDto.setQuantityInTransferProcess(dispatcherLog.getAvailableQuantity());
		...
		// if(dispatcherLog.getPrice() == null){
		// dispatcherLogDto.setPrice("");
		// }else{
		// dispatcherLogDto.setPrice(dispatcherLog.getPrice().toString());
		// }
	...
	public DispatcherLog mapDispatcherEntity(BulkRequestResponseDto bulkRequestResponseDto,
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatcherEntity--------------------started-->");
	...
	public DispatcherLog mapDispatcherEntityForWareHouseMove(BulkRequestResponseDto bulkRequestResponseDto,
			InventoryEntranceDto inventoryEntranceDto, Principal principal, String referenceNo) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatcherEntityForWareHouseMove--------------------started-->");
		DispatcherLog dispatchLog = new DispatcherLog();
		dispatchLog.setRequestedTo(bulkRequestResponseDto.getProjectName());
		dispatchLog.setReferenceNo(referenceNo);
		dispatchLog.setQuantity(inventoryEntranceDto.getQuantity());
		...
		if (inventoryAssign != null) {
			...
			dispatchLog.setInventoryAssign(inventoryAssign);
		}
		Category category = categoryRepository.findByCategoryCode(inventoryEntranceDto.getCategoryCode());
		if (category != null) {
			dispatchLog.setCategory(category);
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatcherEntityForWareHouseMove--------------------Ended-->");
		return dispatchLog;
	}

	/*
	 * private InventoryAssign mapInventoryAssignForId(Long id) {
	 * InventoryAssign inventoryAssign=new InventoryAssign(); logger.info(
	 * "<--DispatchLogManagementServiceImpl-------------mapInventoryAssignForId--------------------started-->"
	 * ); inventoryAssign=inventoryAssignRepository.findOne(id); logger.info(
	 * "<--DispatchLogManagementServiceImpl-------------mapInventoryAssignForId--------------------started-->"
	...
	public List<DispatcherLogDto> getDispatchLogBySiteName(DispatcherLogDto dispatcherLogDto, Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------getDispatchLogBySiteName--------------------started-->");
		List<DispatcherLogDto> dispatcherLogDtoList = new ArrayList<>();
		try {
			List<DispatcherLog> dispatcherLogEntityList = dispatchLogRepository
					.findByRequestedFrom(dispatcherLogDto.getRequestedFrom());
			if (dispatcherLogEntityList != null) {
				for (int j = 0; j < dispatcherLogEntityList.size(); j++) {
					DispatcherLog dispatcherLog = dispatcherLogEntityList.get(j);
					if (dispatcherLog != null) {
	...
	/*
	 * @Override public List<DispatcherLogDto>
	 * getDispatchLogByRequestedTo(DispatcherLogDto dispatcherLogDto, Principal
	 * principal) { logger.info(
	 * "<--DispatchLogManagementServiceImpl-------------getDispatchLogByRequestedTo--------------------started-->"
	 * ); List<DispatcherLogDto> dispatcherLogDtoList=new ArrayList<>(); try {
	 * 
	 * List<WareHouseDto>
	 * warehouseDtoList=userManagementService.getCurrentUserWareHouseDtoList(
	 * principal);
	 * 
	 * List<ProjectDto>
	 * projectDtoList=inventoryManagementService.getCurrentUserProjectDtoList(
	 * principal);
	 * 
	 ...
	 * List<DispatcherLog> dispatcherLogEntityList=dispatchLogRepository.
	 * findByRequestedToAndRequestedFrom(dispatcherLogDto.getRequestedTo(),
	 * wareHouseDto.getWareHouseName()); if(dispatcherLogEntityList!=null) {
	 * for(int j=0;j<dispatcherLogEntityList.size();j++) { DispatcherLog
	 * dispatcherLog=dispatcherLogEntityList.get(j); //if(dispatcherLog!=null &&
	 * dispatcherLog.getAvailableQuantity()>0 &&
	 * dispatcherLog.getStatus().equals(InventoryConstant.PENDING) ||
	 * dispatcherLog.getStatus().equals(InventoryConstant.PARTIALLY_DISPATCHED)
	 * ) if(dispatcherLog!=null && dispatcherLog.getAvailableQuantity()>0) {
	 * dispatcherLogDtoList.add(mapDispatcherLogDto(dispatcherLog)); } } } } }
	 * 
	 * } else if(projectDtoList!=null && !projectDtoList.isEmpty()) { for(int
	 * i=0;i<projectDtoList.size();i++) { ProjectDto
	 * projectDto=projectDtoList.get(i); if(projectDto!=null) {
	 * List<DispatcherLog> dispatcherLogEntityList=dispatchLogRepository.
	 * findByRequestedToAndRequestedFrom(dispatcherLogDto.getRequestedTo(),
	 * projectDto.getName()); if(dispatcherLogEntityList!=null) { for(int
	 * j=0;j<dispatcherLogEntityList.size();j++) { DispatcherLog
	 * dispatcherLog=dispatcherLogEntityList.get(j); //if(dispatcherLog!=null &&
	 * dispatcherLog.getAvailableQuantity()>0 &&
	 * dispatcherLog.getStatus().equals(InventoryConstant.PENDING) ||
	 * dispatcherLog.getStatus().equals(InventoryConstant.PARTIALLY_DISPATCHED)
	 ...
	 * "<--DispatchLogManagementServiceImpl-------------getDispatchLogByRequestedTo--------------------Ended-->"
	 * ); return dispatcherLogDtoList; }
	 */

	@Override
	public List<DispatcherLogDto> getDispatchLogByRequestedToAndRequestedFrom(DispatcherLogDto dispatcherLogDto,
			Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------getDispatchLogByRequestedToAndRequestedFrom--------------------started-->");
		List<DispatcherLogDto> dispatcherLogDtoList = new ArrayList<>();
		try {

			List<DispatcherLog> dispatcherLogEntityList = dispatchLogRepository.findByRequestedToAndRequestedFrom(
					dispatcherLogDto.getRequestedTo(), dispatcherLogDto.getRequestedFrom());
			if (dispatcherLogEntityList != null) {
				for (int j = 0; j < dispatcherLogEntityList.size(); j++) {
					DispatcherLog dispatcherLog = dispatcherLogEntityList.get(j);
					// if(dispatcherLog!=null &&
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------getDispatchLogByRequestedToAndRequestedFrom--------------------Ended-->");
		return dispatcherLogDtoList;
	}
				...
				for (int i = 0; i < bulkRequestResponseDto.getInventoryEntranceItemDto().size(); i++) {
					...
					if (inventoryEntranceDto != null) {
						if (inventoryEntranceDto.getAllocatableQuantity() > 0 && inventoryEntranceDto.getSelected()) {
							...
							dispatcherLog = dispatchLogRepository
									.findOne(bulkRequestResponseDto.getInventoryEntranceItemDto().get(i).getId());
							if (dispatcherLog.getAvailableQuantity() != null
									&& dispatcherLog.getAvailableQuantity() > 0) {
								remaining = dispatcherLog.getAvailableQuantity()
										- inventoryEntranceDto.getAllocatableQuantity();
								dispatcherLog.setAvailableQuantity(remaining);
								dispatcherLog
										.setQuantityInTransferProcess(inventoryEntranceDto.getAllocatableQuantity());
								...
								if (inventoryEntranceDto.getPurpose()
										.equals(InventoryConstant.MOVETOSERVICEREQUESTED)) {
									// dispatcherLog.setStatus(InventoryConstant.ITEM_RECEIVED);
									dispatcherLog.setSubStatus(InventoryConstant.CHALLAN_GENERATED);
									responseDto.setExceptionMessage(InventoryConstant.MOVEDTOSERVICE);
								}

								if (inventoryEntranceDto.getPurpose().equals(InventoryConstant.MOVETOSCRAPREQUESTED)) {
									dispatcherLog.setSubStatus(InventoryConstant.CHALLAN_GENERATED);
									responseDto.setExceptionMessage(InventoryConstant.MOVEDTOSCRAP);
								}

								if (inventoryEntranceDto.getPurpose().equals(InventoryConstant.MOVETOSERVICEREQUESTED)
										|| inventoryEntranceDto.getPurpose()
												.equals(InventoryConstant.MOVETOSCRAPREQUESTED)) {
									
									InventoryAssign inventoryAssign = dispatcherLog.getInventoryAssign();
									if (inventoryAssign != null) {
										Integer test = inventoryAssign.getAllocatedQty()
												- dispatcherLog.getAcceptedQuantity();
								...
								dispatcherLog.setPrice(
										MasterDateUtil.ConvertStringtoInteger(inventoryEntranceDto.getPrice()));

								// if(!inventoryEntranceDto.getPrice().equals("-")){
								// dispatcherLog.setPrice(MasterDateUtil.ConvertStringtoInteger(inventoryEntranceDto.getPrice()));
								// }else{
								// dispatcherLog.setPrice(0);
								// }
								//
								...
								dispatcherLog
										.setVehicleNumber(bulkRequestResponseDto.getDeliveryChallanDto().getVehicleNo()
												+ "-" + bulkRequestResponseDto.getDeliveryChallanDto().getVehicleNo1()
												+ "-" + bulkRequestResponseDto.getDeliveryChallanDto().getVehicleNo2()
												+ "-" + bulkRequestResponseDto.getDeliveryChallanDto().getVehicleNo3());

								dispatcherLog.setDispatchedQuantity(dispatcherLog.getDispatchedQuantity()
										+ inventoryEntranceDto.getAllocatableQuantity());

								if (dispatcherLog.getDeliveryChallanEntityList() != null
										&& !dispatcherLog.getDeliveryChallanEntityList().isEmpty()) {
									dispatcherLog.getDeliveryChallanEntityList().add(mapDeliveryChallanEntity(
											bulkRequestResponseDto.getDeliveryChallanDto(), challanNo,
											inventoryEntranceDto, dispatcherLog, bulkRequestResponseDto, principal));
								} else {
									deliveryChallanEntitiesList.add(mapDeliveryChallanEntity(
											bulkRequestResponseDto.getDeliveryChallanDto(), challanNo,
											inventoryEntranceDto, dispatcherLog, bulkRequestResponseDto, principal));
	...
	public DeliveryChallanEntity mapDeliveryChallanEntity(DeliveryChallanDto deliveryChallanDto, String challanNo,
			InventoryEntranceDto inventoryEntranceDto, DispatcherLog dispatcherLog,
			BulkRequestResponseDto bulkRequestResponseDto, Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------DeliveryChallanEntity--------------------started-->");
		DeliveryChallanEntity deliveryChallanEntity = null;

		// DispatcherLogDto deliveryChallanEntity2 =
		// deliveryChallanDto.getDispatcherLogDto();
		...
		try {
			...
			if (deliveryChallanDto.getDcDate() != null) {
				deliveryChallanEntity
						.setDispatchDate(MasterDateUtil.convertStringToDateWithoutTime(deliveryChallanDto.getDcDate()));
			}
			...
			deliveryChallanEntity.setRequestTo(deliveryChallanDto.getRequestTo());
			// deliveryChallanEntity.setRequestTo(dispatcherLog2.getRequestedTo())
			// ;
			...
			deliveryChallanEntity
					.setVehicleNo(deliveryChallanDto.getVehicleNo() + "-" + deliveryChallanDto.getVehicleNo1() + "-"
							+ deliveryChallanDto.getVehicleNo2() + "-" + deliveryChallanDto.getVehicleNo3());
			deliveryChallanEntity.setGstIn(deliveryChallanDto.getGstIn());
			...
			if (inventoryEntranceDto.getPurpose().equals(InventoryConstant.MOVETOSCRAPREQUESTED)) {
				...
				deliveryChallanEntity.setSubStatus(InventoryConstant.CHALLAN_GENERATED);
				mapDispatchLogToScrapEntity(inventoryEntranceDto, bulkRequestResponseDto, principal);
				...
				mapDispatchLogToServiceEntity(inventoryEntranceDto, bulkRequestResponseDto, principal);
			}
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------DeliveryChallanEntity--------------------Ended-->");
		return deliveryChallanEntity;
	}

	@Override
	public ResponseDto acceptInventoryByChallaNo(BulkRequestResponseDto bulkRequestResponseDto, Principal principal) throws Exception {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------acceptInventoryByChallaNo--------------------started-->");
		...
		if (projectDtoList != null && !projectDtoList.isEmpty()) {
			for (int j = 0; j < projectDtoList.size(); j++) {
				...
				if (projectDto != null)
				// if(projectDto.getName()==
				// bulkRequestResponseDto.getProjectName())
				{
					for (int i = 0; i < bulkRequestResponseDto.getInventoryEntranceItemDto().size(); i++) {
						...
						InventoryEntranceDto inventoryEntranceDto = bulkRequestResponseDto.getInventoryEntranceItemDto()
								.get(i);
						if (inventoryEntranceDto != null
								&& projectDto.getName().equals(inventoryEntranceDto.getRequestedTo())) {
							DispatcherLog dispatcherLog = dispatchLogRepository.findOne(inventoryEntranceDto.getId());
							
								if (dispatcherLog.getStatus().equals(InventoryConstant.ITEM_RECEIVED))
							{
								throw new Exception(InventoryConstant.ALREADY_PROCESSED);
							}
								if (dispatcherLog != null) {
									
								dispatcherLog.setStatus(InventoryConstant.ITEM_RECEIVED);

								List<DeliveryChallanEntity> deliveryChallanEntitiesList = dispatcherLog
										.getDeliveryChallanEntityList();
								...
								if (deliveryChallanEntitiesList != null) {
									...
									for (int m = 0; m < dbsize; m++) {
										DeliveryChallanEntity deliveryChallanEntity = deliveryChallanEntitiesList
												.get(m);
										if (deliveryChallanEntity != null) {
											if (deliveryChallanEntity.getChallanNo() != null && bulkRequestResponseDto
													.getChallanNo().equals(deliveryChallanEntity.getChallanNo())) {
												deliveryChallanEntity.setStatus(InventoryConstant.ITEM_RECEIVED);
												receivedQuantity = receivedQuantity
														- deliveryChallanEntity.getQuantity();

												InventoryEntrance inventoryEntrance = dispatcherLog
														.getInventoryEntrance();
												if (inventoryEntrance != null) {
													Integer test = inventoryEntrance.getQuantity()
															- dispatcherLog.getQuantityInTransferProcess();
													System.out.println(test);
			...
			for (int i = 0; i < bulkRequestResponseDto.getInventoryEntranceItemDto().size(); i++) {
				...
				if (inventoryEntranceDto != null) {
					if (inventoryEntranceDto.getPurpose().equals(InventoryConstant.ASSIGN_TO_PROJECT)) {
						responseDto = mapDispatchLogToInventoryAssignEntity(inventoryEntranceDto,
								bulkRequestResponseDto, principal);
					}
				}
			}
			// responseDto.setStatus(InventoryConstant.SUCCESS);
			...
			if (wareHouseDtoList != null && !wareHouseDtoList.isEmpty()) {
				for (int j = 0; j < wareHouseDtoList.size(); j++) {
					...
					if (warehouseDto != null) {
						for (int i = 0; i < bulkRequestResponseDto.getInventoryEntranceItemDto().size(); i++) {
							deliveryChallanEntityMapperList = new ArrayList<>();
							InventoryEntranceDto inventoryEntranceDto = bulkRequestResponseDto
									.getInventoryEntranceItemDto().get(i);
							if (inventoryEntranceDto != null
									&& warehouseDto.getWareHouseName().equals(inventoryEntranceDto.getRequestedTo())) {
								DispatcherLog dispatcherLog = dispatchLogRepository
										.findOne(inventoryEntranceDto.getId());
								if (dispatcherLog != null) {
									dispatcherLog.setStatus(InventoryConstant.ITEM_RECEIVED);
										...
										if (deliveryChallanEntity != null) {
											...
											if (deliveryChallanEntity.getChallanNo() != null && bulkRequestResponseDto
												...
												if (inventoryAssign != null) {
													...
													Integer test1 = inventoryAssign.getAllocatedQty()
															- dispatcherLog.getQuantityInTransferProcess();
													System.out.println(test1);
													inventoryAssign.setAllocatedQty(test1);
													inventoryAssignRepository.saveAndFlush(inventoryAssign);

												}
												InventoryEntrance inventoryEntrance = inventoryAssign
														.getInventoryEntrance();
												if (inventoryEntrance != null) {
													// inventoryEntranceDto.getAvailableQuantity(inventoryEntrance.setQuantity());
													inventoryEntrance.setAvailableQuantity(
															inventoryEntrance.getAvailableQuantity());
													inventoryEntrance.setQuantity(inventoryEntrance.getQuantity()
															+ dispatcherLog.getQuantityInTransferProcess());
													inventoryStockRepository.saveAndFlush(inventoryEntrance);
								 ...
								 * dispatcherLog.setQuantityInTransferProcess(
								 * receivedQuantity);
								 * dispatcherLog.setDeliveryChallanEntityList(
								 * deliveryChallanEntityMapperList);
								 * dispatchLogRepository.saveAndFlush(
								 * dispatcherLog);
								 */
					...
					if (warehouseDto != null) {
						for (int i = 0; i < bulkRequestResponseDto.getInventoryEntranceItemDto().size(); i++) {
							...
							if (inventoryEntranceDto != null
									&& warehouseDto.getWareHouseName().equals(inventoryEntranceDto.getRequestedTo())) {
								if (inventoryEntranceDto.getPurpose()
										.equals(InventoryConstant.MOVETOWAREHOUSEREQUESTED)) {
									responseDto = mapDispatchLogtoInventoryAssignForWareHouseMove(inventoryEntranceDto,
											warehouseDto, principal);
								} else if (inventoryEntranceDto.getPurpose()
										.equals(InventoryConstant.MOVETOSERVICEREQUESTED)) {
									responseDto = mapDispatchLogToServiceEntity(inventoryEntranceDto,
											bulkRequestResponseDto, principal);
								}

								else if (inventoryEntranceDto.getPurpose()
										.equals(InventoryConstant.MOVETOSCRAPREQUESTED)) {
									responseDto = mapDispatchLogToScrapEntity(inventoryEntranceDto,
											bulkRequestResponseDto, principal);
								}
							}
						}
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------acceptInventoryByChallaNo--------------------Ended-->");
		return responseDto;
	}

	private ResponseDto mapDispatchLogtoInventoryAssignForWareHouseMove(InventoryEntranceDto inventoryEntranceDto,
			WareHouseDto warehouseDto, Principal principal) {

		ResponseDto responseDto = new ResponseDto();
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatchLogtoInventoryAssignForWareHouseMove--------------------Ended-->");
		if (inventoryEntranceDto != null
				&& warehouseDto.getWareHouseName().equals(inventoryEntranceDto.getRequestedTo())) {

			InventoryEntrance inventoryEntrance = inventoryStockRepository.findByInventoryNumberAndWareHouseSiteName(
					inventoryEntranceDto.getInventoryNumber(), warehouseDto.getWareHouseName());
			if (inventoryEntrance != null) {
				inventoryEntrance.setAvailableQuantity(
						inventoryEntrance.getAvailableQuantity() + inventoryEntranceDto.getQuantity());
				inventoryStockRepository.saveAndFlush(inventoryEntrance);
			} else {
				inventoryEntrance = new InventoryEntrance();
				inventoryEntrance = inventoryStockRepository
						.findByInventoryNumber(inventoryEntranceDto.getInventoryNumber());
				if (inventoryEntrance != null) {
	...
	private ResponseDto mapDispatchLogToInventoryAssignEntity(InventoryEntranceDto inventoryEntranceDto,
			BulkRequestResponseDto bulkRequestResponseDto, Principal principal) {

		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatchLogToInventoryAssignEntity--------------------started-->");
		ResponseDto responseDto = new ResponseDto();
		DispatcherLog dispatcherLog = dispatchLogRepository.findOne(inventoryEntranceDto.getId());

		// here add quantity which is in deliverychallan entity not in dispatch
		// log
		for (int k = 0; k < dispatcherLog.getDeliveryChallanEntityList().size(); k++) {
			...
			if (deliveryChallanEntity != null) {
				if (deliveryChallanEntity.getChallanNo().equals(bulkRequestResponseDto.getChallanNo())) {
					...
					inventoryAssign.setInventoryNumber(dispatcherLog.getInventoryNumber());
					...
					if (dispatcherLog.getLocation().equals(InventoryConstant.PROJECTSITE)) {
						inventoryAssign.setInventoryNumber(dispatcherLog.getInventoryAssign().getInventoryNumber());
					} else {
					...
					if (dispatcherLog.getLocation().equals(InventoryConstant.PROJECTSITE)) {
						...
						if (dispatcherLog.getInventoryAssign() != null) {
							inventoryAssign
									.setInventoryEntrance(dispatcherLog.getInventoryAssign().getInventoryEntrance());
						}

					} else {

						if (dispatcherLog.getInventoryEntrance() != null) {

							inventoryAssign.setInventoryEntrance(dispatcherLog.getInventoryEntrance());

							// InventoryEntrance inventoryEntrance =
							// inventoryStockRepository
							// .findOne(dispatcherLog.getInventoryEntrance().getId());
							// if (inventoryEntrance != null) {
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatchLogToInventoryAssignEntity--------------------Ended-->");
		return responseDto;
	}

	private ResponseDto mapDispatchLogToScrapEntity(InventoryEntranceDto inventoryEntranceDto,
			BulkRequestResponseDto bulkRequestResponseDto, Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatchLogToScrapEntity--------------------Started-->");
		ResponseDto responseDto = new ResponseDto();
		DispatcherLog dispatcherLog = dispatchLogRepository.findOne(inventoryEntranceDto.getId());
		for (int k = 0; k < dispatcherLog.getDeliveryChallanEntityList().size(); k++) {
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatchLogToScrapEntity--------------------Ended-->");
		return responseDto;
	}

	private ResponseDto mapDispatchLogToServiceEntity(InventoryEntranceDto inventoryEntranceDto,
			BulkRequestResponseDto bulkRequestResponseDto, Principal principal) {
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatchLogToServiceEntity--------------------Started-->");
		ResponseDto responseDto = new ResponseDto();
		DispatcherLog dispatcherLog = dispatchLogRepository.findOne(inventoryEntranceDto.getId());

		if (inventoryEntranceDto.getPurpose().equals(InventoryConstant.MOVETOSERVICEREQUESTED)) {

			ServiceInspection serviceInspection = serviceInspectionRepository
					.findByReferenceNo(dispatcherLog.getReferenceNo());
			...
			serviceTracker.setReferenceNo(dispatcherLog.getReferenceNo());
			...
			serviceTracker.setQuantity(dispatcherLog.getQuantity());
			...
			serviceTracker.setAvailableQty(String.valueOf(dispatcherLog.getQuantity()));
			serviceTracker.setMovedQty("");
			...
			if (dispatcherLog.getInventoryEntrance() != null) {
				serviceTracker.setInventoryEntrance(dispatcherLog.getInventoryEntrance());
			}
		...
		logger.info(
				"<--DispatchLogManagementServiceImpl-------------mapDispatchLogToServiceEntity--------------------Ended-->");
		return responseDto;
	}
