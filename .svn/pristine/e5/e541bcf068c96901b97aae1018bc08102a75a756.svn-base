...
//	List<InventoryEntrance> findBySubCategoryName(String subCategory);
	...
	@Query(value="select i.* from inventory_stock i,subcategory s where i.sub_category_name=:subCategory and i.subcategory_id=s.id  and i.warehouse_sitename=:warehouse_sitename and i.available_quantity > 0 order by i.asset_no",nativeQuery=true)
	List<InventoryEntrance> getMaxQuantity(@Param("subCategory") String subcategory,@Param("warehouse_sitename")String wareHouseOrSiteName);
	...
	List<Object> findByManufacturerName(String name);

	List<Object> findByWareHouseBlockId(Long id);

	List<Object> findBySupplierName(String supplierName);
	
	@Query(value = "select (SELECT C.category_type FROM category C WHERE ID=category_id) AS CategoryType,(SELECT C.category_code FROM category C WHERE ID=category_id) AS CategoryCode,(SELECT C.asset_tracking FROM category C WHERE ID=category_id) AS AssesTracking,(SELECT S.sub_category FROM subcategory S WHERE ID=subcategory_id) AS sub_category,available_quantity,model_code,inventory_number,warehouse_sitename,quantity,creation_date,location from inventory_stock union  SELECT (SELECT C.category_type FROM category C WHERE ID=ia.category_id) AS CategoryType,(SELECT C.category_code FROM category C WHERE ID=category_id) AS CategoryCode,(SELECT C.asset_tracking FROM category C WHERE ID=category_id) AS AssesTracking,(SELECT S.sub_category FROM subcategory S WHERE ID=ia.subcategory_id) AS sub_category,ia.available_qty,ia.model_code,inventory_number,ia.assigned_location,ia.allocated_qty,ia.creation_date,ia.inventory_location FROM inventory_assign ia", nativeQuery = true)
	List<?> findAllInventory();
