ims.controller('manufacturerManagementController',function($scope,$http,InsertService,FetchService,DTOptionsBuilder,$location,UrlConstants,$timeout) {
   ...
   /*  
	...
	$scope.edit = function(Manufacturer_Details)
	{
		...
		console.log(JSON.stringify(Manufacturer_Details));
	...
	{
		location.href=UrlConstants.REDIRECTOFFICEADMINIMS_URL+"Manufacturer";
	}
	 ...
	 $scope.isCountrySelected=function(indiaOrOutside)
	  ...
	  $scope.stdChange = function(val){
	        ...
	        if(angular.isUndefined(val) || angular.isDefined(val)){
	               ...
	               if($scope.manufacturerDto.stdCode || $scope.manufacturerDto.telephoneNumber){
	  ...
	  $scope.faxChange = function(val){
	        ...
	        if(angular.isUndefined(val) || angular.isDefined(val)){
	               ...
	               if($scope.manufacturerDto.fax || $scope.manufacturerDto.fax1){
