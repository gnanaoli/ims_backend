			...
			InsertService.saveToolsRequest($scope.inventoryRequestDto).then(function(response)
					{
				       if(response.status=="success")
				    	   ...
				    	   swal(
									 'Tools Requested',
									 'Your Request With Reference No :'+response.number+'',
				                     'success'
...
$scope.checkrequest= function(location){		
	...
	$scope.saveAcceptToolsRequest=function(val)
	{
		...
		if($scope.acceptTools.$valid)
			...
			if(parseInt($scope.inventoryRequestDto.requestedQuantity) < parseInt($scope.inventoryRequestDto.quantity))
				{
	  				swal(
				...
				if($scope.inventoryRequestDto.status == "REJECTED")
				...
				else
					{
						...
						if(parseInt($scope.inventoryRequestDto.requestedQuantity) < parseInt($scope.inventoryRequestDto.quantity) && parseInt($scope.inventoryRequestDto.quantity) != 0)
						{
							$scope.inventoryRequestDto.status = "PARTIALLY ACCEPTED";
							$scope.readOnly = true;
						}
						
						if(parseInt($scope.inventoryRequestDto.quantity) == 0)
						{
					...
					InsertService.saveAcceptToolsRequest($scope.inventoryRequestDto).then(function(response)
							{
						       if(response.status=="success")
						    	   {
						    	   swal( 'Tools Accepted','Your Request With Reference No :'+response.number+', Has Been Moved,Please Check Your Dispatch Log', 'success').then(function(isConfirm) {
						                  		if (isConfirm) {
...
if(parseInt($scope.inventoryRequestDto.requestedQuantity) < parseInt(qty))
		{
	  			swal(
					'Can not Accept more than Requested Quantity',
                    '',
	...
	if(parseInt($scope.inventoryRequestDto.requestedQuantity) > parseInt(qty) && parseInt(qty) != 0)
	{
	$scope.inventoryRequestDto.status = "PARTIALLY ACCEPTED";
	$scope.readOnly = true;
	}
	
	if($scope.inventoryRequestDto.location != 'ALL')
		{
			if(parseInt(qty) == 0)
			{
			$scope.inventoryRequestDto.status = "REJECTED";
