...
public class InventoryController3 {
	...
	public ResponseEntity<List<InventoryEntranceDto>> getAllInventoryManagement(Principal principal)
	{
		...
		//List<InventoryAssignDto> inventoryAssignDtos=null;
...
/*	
	@RequestMapping(value="/getAllInventoryUserCount")
	public ResponseEntity<List<InventoryAssignDto>> getAllInventoryAssign(Principal principal)
	{
		List<InventoryAssignDto> inventoryAssignDtoList=null;
		
		ModelAndView model=new ModelAndView("Inventory_mgmt_table");
		logger.info("<--inventoryController3-------------getAllInventoryManagement--------------------started-->");
	try {
		inventoryAssignDtoList=inventoryManagementService.getAllInventoryAssignList(principal);
		System.out.println("inventoryManagementDtoList==>"+new ObjectMapper().writeValueAsString(inventoryAssignDtoList));
		model.addObject("inventoryAssignDtoList", inventoryAssignDtoList);
	} catch (Exception e) {
		e.printStackTrace();
		new ResponseEntity<List<InventoryAssignDto>>(inventoryAssignDtoList,HttpStatus.BAD_REQUEST);
	}
		logger.info("<--inventoryController3-------------getAllInventoryManagement--------------------ended-->");
		return new ResponseEntity<List<InventoryAssignDto>>(inventoryAssignDtoList,HttpStatus.OK);
	}*/
	...
	public ResponseEntity<ResponseDto> getInventoryAssignByChallanNo(@RequestBody InventoryEntranceDto inventoryEntranceDto,Principal principal)
	{
		...
		try {
			...
			{
				/*if(inventoryEntranceDto.getProjectItemDto()!= null){
	...
	public ResponseEntity<ResponseDto> getNotificationData(Principal principal)
	{
		...
		ResponseDto responseDto=new ResponseDto(); 
		try {
			responseDto.setResponseDtoList(notificationService.getNotificationData(principal));
			System.out.println("getAllDispatchLog==>"+new ObjectMapper().writeValueAsString(responseDto));
		} catch (Exception e) {
			responseDto.setExceptionMessage(e.getMessage());
			e.printStackTrace();
	...
	@RequestMapping(value="/updateNotificationStatus",method = RequestMethod.POST)
	public ResponseEntity<ResponseDto> acceptInventoryFromWarehouse(@RequestBody NotificationDto notificationDto,Principal principal)
	{
		logger.info("<--inventoryController3-------------updateNotificationStatus--------------------started-->");
		ResponseDto responseDto=new ResponseDto();
		try {
			
			responseDto=notificationService.updateNotificationStatus(notificationDto,principal);
