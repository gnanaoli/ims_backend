			...
			if (category.getCategoryType().equalsIgnoreCase(categoryDto.getCategoryType())) {
				if (category.getCategoryCode().equalsIgnoreCase(categoryDto.getCategoryCode())) {
					...
					category.setCreationDate(
							MasterDateUtil.convertStringToTimeStampDate(categoryDto.getCreationDate()));
					...
					/*
					 * List<InventoryAssign>
					 * inventoryAssignList=category.getInventoryAssign();
					 * if(inventoryAssignList.size()>0) { throw new
					 * Exception(InventoryConstant.
					 * THIS_CATEGORY_ASSIGN_TO_THE_PROJECT_CAN_NOT_BE_EDIT); }
					 * if(category.getInventoryAssign().size()>0)
					 * 
					 * { throw new Exception(InventoryConstant.
					 * THIS_CATEGORY_ASSIGN_TO_THE_PROJECT_CAN_NOT_BE_EDIT); }
					 */
				...
				category1 = categoryRepository.getCategoryCodeExceptNotInGivenId(categoryDto.getId(),
