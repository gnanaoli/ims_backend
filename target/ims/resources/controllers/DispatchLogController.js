ims.controller('dispatchLogController',function($scope,$http,TestService,$window,FetchService,InsertService,$location,UrlConstants) {
       ...
       $scope.addToolsForTransfer=function()
       {
         ...
         if($scope.dispatchLogDtoForEachforAddList.length>0)
        	 ...
        	 var qtycheck = false;
        	 	...
        	 	for(i=0;i<$scope.dispatchLogDtoForEachforAddList.length;i++)
        	 		{
        	 			if(parseInt($scope.dispatchLogDtoForEachforAddList[i].price) > 0 && $scope.dispatchLogDtoForEachforAddList[i].price != "")
        	 				{
        	 					qtycheck = true;
        	 				}
        	 			else
  ...
  /*this method is used dispatch tools to project by warehouse administrator*/
	 $scope.generateChallanNo=function(formvalid)
	 {
		 if(formvalid)
			 {
			  var object={
			  ...
			  InsertService.generateChallanNo(object).then(function(response){
				  ...
				  if(response.status=='success')
					  ...
					  if(response.exceptionMessage == 'MOVED TO SERVICE' || response.exceptionMessage == 'MOVED TO SCRAP')
					  {
					  swal(
					...
					if(angular.isDefined(receiveToolsList))
						{
							    InsertService.acceptInventoryFromWarehouseOrProjectSite(object).then(function(response){
							    	if(response.status=="ASSIGN TO PROJECT" || response.status=="success" || response.status=="ASSIGN TO WAREHOUSE")
							    	   {
							    		swal(
											      'Received!',
							    	...
							    	if(response.exceptionMessage=="ALREADY PROCESSED")
							    	   {
							    		swal(
							    				'',
							    			      'ALREADY PROCESSED',
