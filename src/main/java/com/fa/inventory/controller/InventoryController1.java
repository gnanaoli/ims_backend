...
public class InventoryController1 {
	...
	public ModelAndView CategoryList(HttpServletRequest request, HttpServletResponse response,
			Authentication authentication, Principal principal, ModelMap map) {
	...
	public ModelAndView editCategory(HttpServletRequest request, HttpServletResponse response, Principal principal)
			throws Exception {
		...
		logger.info(
				"Exit into inventoryController1 -- fetched values for edit category => " + request.getParameter("id"));
		return model;
	}

	/*
	 * @RequestMapping(value = "/getCategory") public
	 * ResponseEntity<ResponseDto> getGategory(HttpServletRequest request,
	 * HttpServletResponse response,Principal principal) { logger.info(
	 * "<--inventoryController1-------------getAllProject--------------------started-->"
	 * ); ResponseDto responseDto=new ResponseDto(); CategoryDto
	 * categoryDto=null; try { categoryDto=new CategoryDto();
	 * categoryDto=categoryService.getCategory(request.getParameter("id"),
	 * principal);; responseDto.setResponseDto(categoryDto);
	 * System.out.println("responseDto==>"+new
	 * ObjectMapper().writeValueAsString(responseDto)); } catch (Exception e) {
	 * e.printStackTrace(); } logger.info(
	 * "<--inventoryController1-------------getAllProject--------------------Ended-->"
	 * ); return new ResponseEntity<ResponseDto>(responseDto, HttpStatus.OK); }
	 */
	...
	public ResponseEntity<ResponseDto> saveProject(@RequestBody ProjectDto ProjectDto, Principal principal) {
		...
		try {
			 ...
			 * responseDto.setStatus(InventoryConstant.SUCCESS); } else{
			 * responseDto.setStatus(InventoryConstant.FAILURE); }
			 * System.out.println("responseprojectDto==>"+new
			 * ObjectMapper().writeValueAsString(ProjectDto)); } catch
	...
	public ModelAndView editWareHouse(HttpServletRequest request, HttpServletResponse response, Principal principal)
			throws Exception {
	...
	public ModelAndView editManufacturer(HttpServletRequest request, HttpServletResponse response, Principal principal)
		...
		try {
			...
			manufacturerDto = manufacturerService.getManufacturer(Long.parseLong(request.getParameter("id")),
					principal);
		...
		logger.info(
				"Exit From inventoryController1 -- fetched values for edit category => " + request.getParameter("id"));
		return model;
	}
	...
	public ResponseEntity<ResponseDto> getCurrentUserWareHouseDropDown(Principal principal) {
		ResponseDto responseDto = new ResponseDto();
		logger.info(
				"<--inventoryController1-------------getCurrentUserWareHouseDropDown--------------------started-->");
		try {
	...
	public ResponseEntity<ResponseDto> saveToolsRequest(@RequestBody InventoryRequestDto inventoryRequestDto,
			Principal principal) {
		ResponseDto responseDto = new ResponseDto();
		String status = null;
...
//			status = toolsRequestService.saveToolsRequest(inventoryRequestDto, principal);
			responseDto = toolsRequestService.saveToolsRequest(inventoryRequestDto, principal);
//			if (status.equals(InventoryConstant.SUCCESS)) {
//				responseDto.setStatus(InventoryConstant.SUCCESS);
//			} else {
//				responseDto.setStatus(InventoryConstant.FAILURE);
//			}
	...
	public ModelAndView RequestTools(HttpServletRequest request, HttpServletResponse response, Principal principal) {
		...
		try {
			/*
			 * inventoryRequestDto.setCategory(request.getParameter(
			 * "categoryType"));
			 * inventoryRequestDto.setSubCategory(request.getParameter(
			 * "subCategory"));
			 * inventoryRequestDto.setModelCode(request.getParameter("modelCode"
			 * )); if(null!=(request.getParameter("quantity"))); {
			 * inventoryRequestDto.setQuantity(Integer.valueOf(request.
			 * getParameter("quantity"))); }
			 * 
			 * inventoryRequestDto.setWareHouseSiteName(request.getParameter(
			 * "site"));
			 */
			if (request.getParameter("id") != null) {
				System.out.println("id" + request.getParameter("id"));
				...
				if (request.getParameter("location").equalsIgnoreCase(InventoryConstant.PROJECTSITE)) {
					...
					inventoryEntranceDto = inventoryManagementService
							.getInventoryFromAssignById(request.getParameter("id"), principal);
				}
				...
				if (request.getParameter("location").equalsIgnoreCase(InventoryConstant.WAREHOUSE)) {
					...
					inventoryEntranceDto = inventoryManagementService.getInventoryById(request.getParameter("id"),
							principal);
				}
			}
	...
	public ModelAndView viewTools(HttpServletRequest request, HttpServletResponse response, Principal principal)
			throws Exception {
	...
	public ResponseEntity<List<InventoryEntranceDto>> findtool(@RequestBody FindToolsDto findToolsDto,
			Principal principal) {
	...
	public ModelAndView getToolsRequestById(HttpServletRequest request, HttpServletResponse response,
			Principal principal) {
	...
	public ResponseEntity<ResponseDto> saveAcceptToolsRequest(@RequestBody InventoryRequestDto inventoryRequestDto,
...
//		String status = null;
		...
		try {
			...
			responseDto = toolsRequestService.saveAcceptToolsRequest(inventoryRequestDto, principal);
			// status =
			// toolsRequestService.saveAcceptToolsRequest(inventoryRequestDto,
			// principal);
			// if (status.equals(InventoryConstant.SUCCESS)) {
			// responseDto.setStatus(InventoryConstant.SUCCESS);
	...
	public ResponseEntity<ResponseDto> SubCategoryList(HttpServletRequest request, HttpServletResponse response,
			Authentication authentication, Principal principal, ModelMap map) {
	...
	public ResponseEntity<ResponseDto> getReport(@RequestBody InventoryEntranceDto reportDto, Principal principal) {
		/*
		 * String A= reportDto.getFromDate(); String B=reportDto.getToDate();
		 * String fromDate=A+" 00:00:00"; String toDate=B+" 12:00:00";
		 */
		...
		try {
			...
			List<String> projectAndWareHouseName = new ArrayList<>();

			if (reportDto.getProjectItemDto() != null && !reportDto.getProjectItemDto().isEmpty()) {
				List<ProjectDto> projectDtoList = new ArrayList<>();

				projectDtoList = reportDto.getProjectItemDto();
				if (projectDtoList != null) {

					for (int k = 0; k < projectDtoList.size(); k++) {
						ProjectDto projectDto = reportDto.getProjectItemDto().get(k);
						if (projectDto != null && null != projectDto.getSelected()) {

							projectAndWareHouseName.add(projectDto.getName());
						}
					}
				}
			}
			if (reportDto.getWareHouseDto() != null && !reportDto.getWareHouseDto().isEmpty()) {
				List<WareHouseDto> warehouseDtoList = new ArrayList<>();

				warehouseDtoList = reportDto.getWareHouseDto();
				if (warehouseDtoList != null) {

					for (int k = 0; k < warehouseDtoList.size(); k++) {
						WareHouseDto wareHouseDto = reportDto.getWareHouseDto().get(k);
						if (wareHouseDto != null && null != wareHouseDto.getSelected()) {

							projectAndWareHouseName.add(wareHouseDto.getWareHouseName());
						}
					}
				}
			}
			...
			if (reportDto.getReportMenu().equals(InventoryConstant.INVENTORY_REPORT)) {
				responseDto.setResponseDtoList(
						reportService.inventoryReport(reportDto, projectAndWareHouseName, principal));
			} else if (reportDto.getReportMenu().equals(InventoryConstant.SCRAP_REPORT)) {
				responseDto
						.setResponseDtoList(reportService.scrapReport(reportDto, projectAndWareHouseName, principal));
			} else if (reportDto.getReportMenu().equals(InventoryConstant.REQUESTS_REPORT)) {
				responseDto.setResponseDtoList(
						reportService.requestsReport(reportDto, projectAndWareHouseName, principal));
			} else if (reportDto.getReportMenu().equals(InventoryConstant.SERVICE_REPORT)) {
				responseDto
						.setResponseDtoList(reportService.serviceReport(reportDto, projectAndWareHouseName, principal));
			} else if (reportDto.getReportMenu().equals(InventoryConstant.ELCB_REPORT)) {
				responseDto.setResponseDtoList(reportService.elcbReport(reportDto, projectAndWareHouseName, principal));
			} else if (reportDto.getReportMenu().equals(InventoryConstant.WORKMEN_DETAILS_REPORT)) {
				responseDto.setResponseDtoList(
						reportService.worksMenDetailsReport(reportDto, projectAndWareHouseName, principal));
			} else if (reportDto.getReportMenu().equals(InventoryConstant.INVENTORY_INWORDS)) {
				responseDto.setResponseDtoList(
						reportService.inventoryInwordsReport(reportDto, projectAndWareHouseName, principal));
			} else if (reportDto.getReportMenu().equals(InventoryConstant.INVENTORY_OUTWORDS)) {
				responseDto.setResponseDtoList(
						reportService.inventoryOutwordsReport(reportDto, projectAndWareHouseName, principal));
			}
	...
	public ResponseEntity<ResponseDto> getallhistoryoftask(@RequestBody ProjectsTasksDto projectsTaskDto,
			Principal principal) {
	...
	public ResponseEntity<ResponseDto> saveDesignation(@RequestBody DesignationDto designationDto,
			Principal principal) {
		ResponseDto responseDto = new ResponseDto();
		String status = null;
	...
	public ResponseEntity<ResponseDto> saveSafetyStatus(@RequestBody SafetyStatusDto safetyStatusDto,
			Principal principal) {
		ResponseDto responseDto = new ResponseDto();
		String status = null;
	...
	public ResponseEntity<ResponseDto> getalldeliverychallan(@RequestBody InventoryEntranceDto reportDto,
			Principal principal) {
		/*
		 * String A= reportDto.getFromDate(); String B=reportDto.getToDate();
		 * String fromDate=A+" 00:00:00"; String toDate=B+" 12:00:00";
	 ...
	 * public ResponseEntity<ResponseDto> saveAcceptToolsRequest(@RequestBody
	 * InventoryRequestDto inventoryRequestDto,Principal
	 * principal,Authentication authentication) { ResponseDto responseDto=new
	 * ResponseDto(); String status=null; logger.info(
	 * "<--inventoryController1-------------saveAcceptToolsRequest--------------------started-->"
	 * ); try { System.out.println("request dto==>"+new
	 * ObjectMapper().writeValueAsString(inventoryRequestDto));
	 * 
	 * status=toolsRequestService.saveAcceptToolsRequest(inventoryRequestDto,
	 * principal); if(status.equals(InventoryConstant.SUCCESS)) {
	 * responseDto.setStatus(InventoryConstant.SUCCESS); }
	 * 
	 * System.out.println("responseDto==>"+new
	 * ObjectMapper().writeValueAsString(inventoryRequestDto)); } catch
	 * (Exception e) { responseDto.setExceptionMessage(e.getMessage());
	 * e.printStackTrace(); }
	 * 
	 * logger.info(
	 * "<--inventoryController1-------------saveAcceptToolsRequest--------------------ended-->"
	 * ); return new ResponseEntity<ResponseDto>(responseDto, HttpStatus.OK); }
	 */
	/*
	 * @RequestMapping(value = "/moveBackServiedTools", method =
	 * RequestMethod.GET) public String sayHelloAgain(ModelMap model) {
	 * model.addAttribute("greeting", "Hello World Again, from Spring 4 MVC");
	 * return "MovedServiedTools.jsp"; }
	 */

	@RequestMapping(value = "/moveBackServiedTools", method = RequestMethod.POST)
	public ModelAndView moveBackServiedTools(HttpServletRequest request, HttpServletResponse response,
			Principal principal) {
	...
	/*@RequestMapping(value = "/getAllNotificationDetails")
	public ResponseEntity<ResponseDto> getAllNotificationDetails(Principal principal) {
		logger.info("<--inventoryController1-------------getAllNotification--------------------started-->");
		ResponseDto responseDto = new ResponseDto();
		List<NotificationDto> notificationDtoList= null;
		try {
			notificationDtoList = new ArrayList<>();
			notificationDtoList= notificationService.getAllNotification(principal);
			responseDto.setResponseDtoList(notificationDtoList);
			System.out.println("responseDto==>" + new ObjectMapper().writeValueAsString(responseDto));
		} catch (Exception e) {
			e.printStackTrace();
		}
		logger.info("<--inventoryController1-------------getAllNotification--------------------Ended-->");
		return new ResponseEntity<ResponseDto>(responseDto, HttpStatus.OK);
	}*/
