	...
	public Authentication authenticate(Authentication auth) throws AuthenticationException {
		...
		if (user.getEmployee() != null) {
			if (user.getEmployee().getResignedDate() != null) {
				logger.error(
						" LoginAuthenticationProvider -- authenticate -- ResignedUserNotAllowedException " + username);
				throw new UsernameNotFoundException("You are not an authenticated user");
			}

		}

		// String encryptPasswrod=fasencryptdecrypt.encryptionText(password,
		// "FAS");
		...
		if (user.getLastLoginDateTime() != null) {
			usersessionDto.setLastLoginDateTime(simpleDateFormat.format(user.getLastLoginDateTime()));
		} else {
			usersessionDto.setLastLoginDateTime(simpleDateFormat.format(new Date()));
		}
