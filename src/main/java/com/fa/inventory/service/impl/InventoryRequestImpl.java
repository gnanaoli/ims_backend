...
public class InventoryRequestImpl implements InventoryRequestService {
	...
	public List<InventoryRequestDto> getNotificationData(Principal principal) {
		...
		if(wareHouseDtosList!=null)
		{
			for(int i=0;i<wareHouseDtosList.size();i++)
			{
				...
				if(wareHouseDto!=null)
				{
					List<Notifications> notificationsList=notificationsRepository.findByRequestedToAndStatus(wareHouseDto.getWareHouseName(),InventoryConstant.NEW);
					if(notificationsList!=null)
					{
						for(int j=0;j<notificationsList.size();j++)
						{
							notificationDtoList.add(mapNoticationsEntityToDto(notificationsList.get(j)));
						}
					}
				}
			}
	...
	public ResponseDto updateNotificationStatus(InventoryRequestDto inventoryRequestDto, Principal principal) {
	  ...
	  /*  @Override
		public List<InventoryRequestDto> getAllNotification(Principal principal) {

			InventoryRequest inventoryRequest= new InventoryRequest();
					List<InventoryRequestDto> inventoryRequestDtoList= new ArrayList<>();	
					
			try {
				List<InventoryRequest> inventoryRequestList= inventoryRequestRepository.findAll();
				
				if (inventoryRequestList.size() > 0) {

					for (InventoryRequest inventoryRequest2: inventoryRequestList) {

						InventoryRequestDto inventoryRequestDto= new InventoryRequestDto();
						inventoryRequestDto.setRequestedFrom(inventoryRequest2.getRequestedFrom());
						inventoryRequestDto.setStatus(inventoryRequest2.getStatus());
						inventoryRequestDto.setCategory(inventoryRequest2.getCategory().getCategoryType());
						inventoryRequestDto.setQuantity(inventoryRequest2.getQuantity());
						inventoryRequestDto.setRequestedTo(inventoryRequest2.getRequestedTo());
						inventoryRequestDtoList.add(inventoryRequestDto);
					}
