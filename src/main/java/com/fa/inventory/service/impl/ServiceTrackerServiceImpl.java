...
public class ServiceTrackerServiceImpl implements ServiceTrackerService {
	...
	public ServiceTrackerDto mapServiceTrackerDtoById(ServiceTracker serviceTracker) {
		...
		try {
			...
			serviceTrackerDto.setCreationDate(MasterDateUtil.convertDateToStringWithTime(serviceTracker.getCreationDate()));
			...
			serviceTrackerDto
					.setLastModifiedDate(MasterDateUtil.convertDateToStringWithTime(serviceTracker.getLastModifiedDate()));
			serviceTrackerDto.setQuantity(serviceTracker.getQuantity().toString());
			...
			serviceTrackerDto.setReferenceNo(serviceTracker.getReferenceNo());
			serviceTrackerDto.setAvailableQty(serviceTracker.getAvailableQty());
			serviceTrackerDto.setMovedQty(serviceTracker.getMovedQty());
	...
	public List<?> getAllServiceDueDateForAllRole(Principal principal) {
		...
		if (hasWareHouseRole) {
			...
			if (warehouseList != null && !warehouseList.isEmpty()) {
				inventoryServiceDueDateEntityList = serviceTrackerRepository
						.getAllWareHouseServiceDueDate(warehouseList);
			}
	...
	public List<?> getAllUserServiceDueDate(Principal principal) {
		...
		logger.info(
				"<--ServiceTrackerServiceISSmpl-------------getAllUserServiceDueDate--------------------Started-->");
		...
		if (projectDtoList != null) {
			List<Object> inventoryServiceDueDateEntityList = serviceTrackerRepository
					.getAllUserServiceDueDate(projectList);
	...
	public List<?> getAllWareHouseServiceDueDate(Principal principal) {
		...
		logger.info(
				"<--ServiceTrackerServiceISSmpl-------------getAllWareHouseServiceDueDate--------------------Started-->");
		...
		logger.info(
				"<--ServiceTrackerServiceISSmpl-------------getAllWareHouseServiceDueDate--------------------Ended-->");

		return inventoryEntranceDtoList;
	}

	@Override
	public ResponseDto getAllServiceTracker(Principal principal, HttpSession session) throws Exception {
		...
		if (hasUserRole) {
			...
			if (projectsDtoList != null) {
				for (int j = 0; j < projectsDtoList.size(); j++) {
					...
					if (project != null) {
						List<ServiceTracker> serviceTrackerEntityList = serviceTrackerRepository
								.findByfromLocation(project.getName());

						if (serviceTrackerEntityList != null) {

							for (int i = 0; i < serviceTrackerEntityList.size(); i++) {

								ServiceTracker serviceTracker = serviceTrackerEntityList.get(i);

								if (serviceTracker != null) {

									if (serviceTracker.getStatus().equals(InventoryConstant.ACTIVE) ||serviceTracker.getStatus().equals(InventoryConstant.PARTIALLY_DISPATCHED)) {

										ServiceTrackerDto serviceTrackerDto = new ServiceTrackerDto();
										serviceTrackerDto = mapServiceTrackerDtoById(serviceTracker);
			...
			if (wareHouseDtoList != null) {
				for (int j = 0; j < wareHouseDtoList.size(); j++) {
					...
					if (wareHouseDto != null) {
						List<ServiceTracker> serviceTrackerEntityList = serviceTrackerRepository
								.findByfromLocation(wareHouseDto.getWareHouseName());
						...
						if (serviceTrackerEntityList != null) {
							...
							for (int i = 0; i < serviceTrackerEntityList.size(); i++) {
								...
								if (serviceTracker != null) {
									...
									if (serviceTracker.getStatus().equals(InventoryConstant.ACTIVE)) {
		...
		responseDto.setResponseDtoList(serviceTrackerDtoList);
