...
public class WorksMenServiceImpl implements WorksMenService {
			...
			if (worksMenDetailsEntityList != null && !worksMenDetailsEntityList.isEmpty()) {
				...
				for (int i = 0; i < worksMenDetailsEntityList.size(); i++) {
					...
					if (worksMenDetailsBlocksEntity != null) {
						...
						worksMenDetailsDto
								.setCreationDate(String.valueOf(worksMenDetailsBlocksEntity.getCreationDate()));
						worksMenDetailsDto
								.setResourceCount(String.valueOf(worksMenDetailsBlocksEntity.getResourceCount()));

						worksMenDetailsDto.setId(worksMenDetailsBlocksEntity.getId());

						worksMenDetailsDto
								.setDate(MasterDateUtil.convertDateToString(worksMenDetailsBlocksEntity.getDate()));
	...
	@Override
	public String saveWorksMenDetails(List<WorksMenDetailsDto> worksMenDetailsDtolist, Principal principal)
			throws Exception {
		...
		if (worksMenDetailsDtolist != null) {
			for (int i = 0; i < worksMenDetailsDtolist.size(); i++) {
				...
				worksMenDetails.setCreatedBy(principal.getName());
							...
							String test=worksMenDetailsDto.getResourceCount();
				
		            	//Float test1= worksMenDetails.setResourceCount();

	
		            	worksMenDetails.setResourceCount(Float.valueOf(test));
				...
				if (worksMenDetailsDto.getWorksMenCategoryDto() != null) {
					for (int k = 0; k < worksMenDetailsDto.getWorksMenCategoryDto().size(); k++) {
						...
						if (worksMenCategoryDto != null) {
							worksMenCategory = worksMenCategoryRepository
									.findByCategory(worksMenCategoryDto.getCategory());
	...
	public String saveWorksMenCategory(WorksMenCategoryDto worksMenCategoryDto, Principal principal) throws Exception {
		...
		if (worksMenCategoryDto != null && worksMenCategoryDto.getId() != null) {
			...
			categoryList = worksMenDetailsRepository.findByCategory(worksMenCategoryDto.getCategory());

			if (categoryList.size() > 0) {
				throw new Exception(InventoryConstant.ALREADY_ASSIGNED);
			}

			if (worksMenCategory.getId() == worksMenCategoryDto.getId()
					&& worksMenCategory.getCategory().equals(worksMenCategoryDto.getCategory())) {
			...
			} else if (worksMenCategory.getId() == worksMenCategoryDto.getId()
					&& !worksMenCategory.getCategory().equals(worksMenCategoryDto.getCategory())) {
	...
	public WorksMenDetailsDto mapWorksMenDto(WorksMenDetails worksMenDetails) {
		...
		try {
			...
			worksMenDetailsDto.setResourceCount(String.valueOf(worksMenDetails.getResourceCount()));
			 ...
			 //Float test2=worksMenDetails.getResourceCount();
			 
			 //worksMenDetailsDto.setResourceCount(Float.valueOf(test2)).floatValue());
