...
public interface DeliveryChallanEntityRepository extends JpaRepository<DeliveryChallanEntity, Long> {
	...
	List<DeliveryChallanEntity> getDeliveryChallanEntityList(@Param("challanNo")String challanNo);
	 ...
	 @Query(value="select * from delivery_challan where status='ITEM DISPATCHED' AND request_to=:requestTo AND sub_status='CHALLAN GENERATED' AND challan_no=:challanNo",nativeQuery=true)
		List<DeliveryChallanEntity> findDeliveryChallanEntityByChallanNo(@Param("challanNo") String challanNo,@Param("requestTo") String requestTo);
