...
public interface InventoryAssignRepository extends JpaRepository<InventoryAssign, Long>{
	...
	@Query(value="select i.* from inventory_assign i,subcategory s where s.sub_category=:subCategory and i.subcategory_id=s.id  and i.assigned_location=:requested_to and i.available_qty > 0 order by i.inventory_number",nativeQuery=true)
	List<InventoryAssign> getMaxQuantity(@Param("subCategory") String subcategory,@Param("requested_to")String requestedTo);
