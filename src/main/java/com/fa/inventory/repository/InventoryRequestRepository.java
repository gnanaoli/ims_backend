...
public interface InventoryRequestRepository extends JpaRepository<InventoryRequest, Long> {
   ...
   // @Query(value="select category_id,subcategory_id,quantity,reference_no,requested_from,requested_to,status,sub_status FROM inventory_request ir  WHERE ir.requested_from =:requestedFrom  UNION ALL select  category_id,subcategory_id,quantity,reference_no,requested_from,requested_to,status,sub_status FROM service_inspection sri WHERE sri.requested_from =:requestedFrom UNION ALL select  category_id,subcategory_id,quantity,reference_no,requested_from,requested_to,status,sub_status FROM scrap_inspection si WHERE si.requested_from =:requestedFrom",nativeQuery = true)
    @Query(value="select category_type,sub_category,model,quantity,reference_no,requested_from,requested_to,ir.status,sub_status,action_date FROM inventory_request ir JOIN category c ON  c.id=ir.category_id  JOIN subcategory s ON s.id=ir.subcategory_id  WHERE ir.requested_from =:requestedFrom UNION ALL select  category_type,sub_category,model,quantity,reference_no,requested_from,requested_to,sri.status,sub_status,action_date FROM service_inspection sri JOIN category c ON  c.id=sri.category_id JOIN subcategory s ON s.id=sri.subcategory_id  WHERE sri.requested_from =:requestedFrom UNION ALL select  category_type,sub_category,model,quantity,reference_no,requested_from,requested_to,si.status,sub_status,action_date FROM scrap_inspection si  JOIN category c ON  c.id=si.category_id JOIN subcategory s ON s.id=si.subcategory_id WHERE si.requested_from =:requestedFrom",nativeQuery = true)
